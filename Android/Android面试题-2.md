...
在Android系统启动时，第一个启动起来的进程就是zygote进程，然后由zygote启动SystemServer，再后就是启动ActivityManagerService，WindowManagerService等系统核心服务，这些服务承载着整个Android系统与客户端程序交互的重担。zygote除了启动系统服务和进程之外，普通的用户进程也由zygote进程fork而来，当一个应用进程启动起来后，就会加载用户在清单文件（AndroidManifest.xml）中配置的默认加载的Activity，此时加载的入口是ActivityThread.main(String[] args)，这个方法就类似与C语言中的main()方法，是整个应用程序的入口。
